 ...
 *   - `externalReferenceId` = `RZP_<providerPaymentId>`, IMMUTABLE — a retry
 ...
 *
 * The four scans, in the order `runPending` executes them:
 ...
 *   4. Expiry sweep               — local expiry when the gateway is slow.
 *
  ...
  await writeAudit({
    ...
    details: {
      ...
      note:
        ...
        "gateway and credit by hand using RZP_<providerPaymentId> as the reference.",
    },
  });
 ...
 *
 * @returns {Promise<{reconcilePending:number, needsManualAction:number, unprocessedEvents:number, oldestAgeMinutes:number|null}>}
 */
async function getBacklogCounts() {
  const [reconcilePending, needsManualAction, unprocessedEvents, oldest] =
    await Promise.all([
