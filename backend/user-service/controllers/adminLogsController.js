/**
 * Admin Logs Controller for User Service
...
class AdminLogsController {
  ...
  static async getAuditLogs(req, res) {
    try {
      ...
      const serviceCursor = cursor
        ? LogCursor.decode(cursor).getCursor("user-service")
        : null;
      ...
      const auditEvents = logs.map((log) =>
        AuditEventDTO.fromUserService(log).toJSON(),
      );
      ...
      // Log access
      logger.info(`Admin accessed user-service audit logs`, {
      ...
      logger.error("Error fetching user-service audit logs:", error);
      res.status(500).json({
        status: "error",
        error: {
          code: "INTERNAL_ERROR",
  ...
  static async getRuntimeLogs(req, res) {
    try {
      ...
      const logFiles = listLogFiles("user-service");
      ...
      logger.info(`Admin accessed user-service runtime logs`, {
        requestingUserId: req.user?.userId,
        file: targetFile,
      ...
      logger.error("Error fetching user-service runtime logs:", error);
      res.status(500).json({
        status: "error",
        error: {
          code: "INTERNAL_ERROR",
  ...
  static async listLogFiles(req, res) {
    try {
      ...
      res.json({
        ...
        data: {
          files: logFiles,
          logDirectory: "logs/user-service",
        },
        meta: {
          service: "user-service",
          timestamp: new Date().toISOString(),
      ...
      logger.error("Error listing user-service log files:", error);
      res.status(500).json({
        status: "error",
        error: {
          code: "INTERNAL_ERROR",
