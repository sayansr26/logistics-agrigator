...
export default function ShipmentDetailPage() {
  ...
  const { data: txnData, isLoading: txnLoading } =
    useGetShipmentTransactionsQuery(id, { skip: !id });
  // The wallet has no per-transaction route, so link to its transaction list
  // filtered to THIS shipment. Every reference we write embeds the shipment id
  // (SHIPMENT_<id>, SHIPMENT_<id>_RERATE_<ts>, ..._REVERSAL, REFUND_<id>) and
  // the wallet matches referenceId as a substring, so the id alone pulls the
  // whole history. userId is carried too so the holder column stays scoped.
  const walletLedgerHref = (() => {
    const params = new URLSearchParams({ referenceId: id });
    if (txnData?.data?.walletUserId) {
      params.set("userId", txnData.data.walletUserId);
    }
  ...
  const handleDownloadLabel = async () => {
    try {
      const blob = await downloadLabel({
        shipmentId: id,
        format: labelFormat,
        copies: labelCopies,
      }).unwrap();
  ...
  // Long courier timelines pushed everything below the fold, so only the most
  // recent few show until expanded.
  // Mirrors the allowlist in labelGenerationService — the branded label is
  // generated by us, so it must not be gated behind provider capabilities.
  const canPrintLabel = ["CREATED", "BOOKED", "PICKED_UP"].includes(
    shipment.status,
  );
  const TIMELINE_PREVIEW_COUNT = 5;
  const visibleTrackingEvents = timelineExpanded
    ? trackingEvents
    : trackingEvents.slice(0, TIMELINE_PREVIEW_COUNT);
  const hiddenTrackingEventCount =
    trackingEvents.length - visibleTrackingEvents.length;
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            ...
            <Card>
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center gap-2">
                  <IndianRupee className="h-5 w-5 text-emerald-600" /> Wallet
                  Transactions
                  {txnData?.data?.walletUserId && (
                    <Link
                      href={walletLedgerHref}
                      className="ml-auto text-xs font-normal text-primary underline underline-offset-2 hover:no-underline"
                    >
                      Open wallet ledger
                    </Link>
                  )}
              ...
              <CardContent className="space-y-3">
                ...
                ) : !txnData?.data?.transactions?.length ? (
                  <p className="text-xs text-muted-foreground">
                    No wallet activity recorded for this shipment.
                  </p>
                ) : (
                  <>
                    {txnData.data.transactions.map((txn, idx) => {
                      const isRefund = txn.kind === "REFUND";
                      const isNone = txn.kind === "NONE";
                      return (
                        <div
                          key={`${txn.adjustmentId || txn.reference || idx}-${idx}`}
                    ...
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span>
                          {formatCurrency(txnData.data.summary.totalDebited)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">
                        ...
                        <span>
                          {formatCurrency(txnData.data.summary.totalRefunded)}
                        </span>
                      </div>
                      <div className="flex justify-between font-semibold text-sm pt-1">
                        <span>Net charged</span>
                        <span>
                          {formatCurrency(txnData.data.summary.netCharged)}
                        </span>
                      </div>
                      {txnData.data.summary.currentPrice !== null &&
                        Math.abs(
                          txnData.data.summary.netCharged -
                            txnData.data.summary.currentPrice,
                        ) > 0.01 && (
                          <div className="flex items-start gap-2 pt-2 text-amber-700 dark:text-amber-500">
                            <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" />
                            <span>
                              Net charged does not match the current price of{" "}
                              {formatCurrency(
                                txnData.data.summary.currentPrice,
                              )}
                              . A settlement may have failed.
                            </span>
                          </div>
                        )}
                    </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {trackingEvents.length > 0 ? (
                  visibleTrackingEvents.map((evt, idx) => (
                    <div
                      key={evt.id || idx}
            ...
            {/* Shipping label — ours, not the courier's, so it is always
                available while the status allows it. */}
            {canPrintLabel && (
              <Card>
                <CardHeader className="pb-4">
                  <CardTitle className="flex items-center gap-2">
                    <FileText className="h-5 w-5 text-orange-600" /> Shipping
                    Label
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <p className="text-xs text-muted-foreground">
                    Branded with your outlet details and your own tracking link.
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <option value="A4">A4 (single)</option>
                        <option value="A4_4">A4 (4 per sheet)</option>
                      </select>
                    </div>
                    <div className="space-y-1">
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {labelFormat} PDF
                        {labelCopies > 1 ? ` · ${labelCopies} copies` : ""}
                      </div>
                    </div>
                  </Button>
                </CardContent>
