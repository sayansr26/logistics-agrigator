                ...
                {[
                  ...
                  <div key={dim.id} className="space-y-2">
                    ...
                    <Input
                      ...
                      value={
                        formData[dim.id as keyof typeof formData] as string
                      }
                      onChange={(e) =>
                        setField(
                          dim.id as keyof typeof formData,
                          e.target.value,
                        )
                      }
                      className={`h-11 text-base ${errors[dim.id as keyof typeof errors] ? "border-red-500 ring-red-200" : "border-gray-300 focus:border-blue-500 focus:ring-blue-200"}`}
                    />
                    <FormError
                      message={errors[dim.id as keyof typeof errors]}
                    />
                  </div>
                ))}
              </div>
