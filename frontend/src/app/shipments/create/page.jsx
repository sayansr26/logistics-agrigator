...
export default function CreateShipmentPage() {
  ...
  const transformFormDataToApiFormat = () => {
    ...
    return {
      ...
      pickupAddress: {
        ...
        addressLine2: "",
        landmark: "",
        city: selectedPickup?.details.address.split(", ")[0] || "",
        state: selectedPickup?.details.address.split(", ")[1] || "",
        pincode: selectedPickup?.details.address.split(", ")[2] || "",
        country: "India",
      },
      deliveryAddress: {
        name: receiverName,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!referenceNo)
      validationErrors.referenceNo = "Reference number is required";
    if (!actualWeight)
      validationErrors.actualWeight = "Actual weight is required";
    if (!pickupAddress)
      validationErrors.pickupAddress = "Pickup address is required";
    if (!productDescription)
      validationErrors.productDescription = "Product description is required";
    if (!receiverName)
      validationErrors.receiverName = "Receiver name is required";
    if (!address) validationErrors.address = "Delivery address is required";
    ...
    if (!isRTO) {
      if (
        ...
        validationErrors.returnAddress =
          "Return address is required when RTO is unchecked";
      }
      if (!returnPincode)
        validationErrors.returnPincode = "Return pincode is required";
      if (!returnCity) validationErrors.returnCity = "Return city is required";
      if (!returnState)
        validationErrors.returnState = "Return state is required";
    }

    // Invoice validation
    if (invoices.length === 0) {
      validationErrors.invoices = "At least one invoice is required";
    } else {
      invoices.forEach((invoice, index) => {
        if (!invoice.invoiceNo) {
          validationErrors[`invoiceNo-${index}`] = "Invoice number is required";
        }
        if (!invoice.invoiceDate) {
          validationErrors[`invoiceDate-${index}`] = "Invoice date is required";
        }
        if (!invoice.invoiceAmt) {
          validationErrors[`invoiceAmt-${index}`] =
    ...
    try {
      ...
      console.log("Submitting shipment data:", apiData);
      ...
      // Call the API
      const response = await shipmentApiService.createShipment(apiData);
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-green-700">
                Your shipment has been created and you will be redirected
                shortly.
              </p>
            </div>
          </div>
              ...
              <div className="text-center py-12">
                ...
                {errors.invoices && (
                  <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                    <p className="text-red-700 text-sm">{errors.invoices}</p>
                  </div>
                )}
                <Button
                  type="button"
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        />
                        <FormError message={errors[`invoiceDate-${index}`]} />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          className={`h-11 text-base ${
                            errors[`invoiceAmt-${index}`]
                              ? "border-red-500 focus:ring-red-200"
                              : "border-purple-200 focus:border-purple-500 focus:ring-purple-200"
                          }`}
                        />
                        <FormError message={errors[`invoiceAmt-${index}`]} />
                      </div>
