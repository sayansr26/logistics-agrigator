...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "shipments" | "financial" | "performance"
  >("overview");
  ...
  const handleTabChange = (
    tab: "overview" | "shipments" | "financial" | "performance",
  ) => {
    setActiveTab(tab);
    setCurrentPage(1);
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="hover:shadow-md transition-shadow">
                ...
                <CardContent>
                  <ProgressChart
                    data={Object.entries(
                      mockShipments.reduce(
                        (acc, shipment) => {
                          acc[shipment.status] =
                            (acc[shipment.status] || 0) + 1;
                          return acc;
                        },
                      ...
                      color: getStatusColor(status as any).includes("green")
                        ? "bg-green-500"
                        : getStatusColor(status as any).includes("blue")
                          ? "bg-blue-500"
                          : getStatusColor(status as any).includes("yellow")
                            ? "bg-yellow-500"
                            : getStatusColor(status as any).includes("red")
                              ? "bg-red-500"
                              : "bg-gray-500",
                    }))}
                    showValues={true}
              ...
              <CardContent>
                <ProgressChart
                  data={Object.entries(
                    ...
                    value: count,
                    maxValue: mockShipments.length,
                    color: getPaymentModeColor(mode as any).includes("green")
                      ? "bg-green-500"
                      : getPaymentModeColor(mode as any).includes("blue")
                        ? "bg-blue-500"
                        : getPaymentModeColor(mode as any).includes("orange")
                          ? "bg-orange-500"
                          : getPaymentModeColor(mode as any).includes("purple")
                            ? "bg-purple-500"
                            : "bg-gray-500",
                  }))}
                  showValues={true}
                    ...
                    {Object.entries(
                      ...
                      <TableRow key={platform} className="hover:bg-muted/50">
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            <Badge
                              className={getPlatformColor(platform as any)}
                            >
                              {platform}
                            </Badge>
                          </div>
...
function ShipmentsAnalyticsTable({
  shipments,
  searchTerm,
}: {
  shipments: any[];
