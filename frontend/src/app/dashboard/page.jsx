        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {quickActions.map((action) => (
              ...
              >
                <div className={`p-2 rounded-lg bg-muted mr-3 ${action.color}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Charge Types</p>
              </div>
            </div>
          </div>
        </CardContent>
...
function OutletDashboard({ user }) {
  // Quick actions for outlet users
  const quickActions = [
    {
  ...
  return (
    <>
      <PageHeader
        title="Dashboard"
        description={`Welcome back, ${user?.firstName || user?.first_name || ""}${user?.lastName || user?.last_name ? " " + (user?.lastName || user?.last_name) : ""}`}
        primaryAction={{
          ...
          <Link key={action.href} href={action.href}>
            <Card className="hover:border-primary/50 hover:shadow-md transition-all cursor-pointer h-full">
              <CardContent className="flex items-center p-6">
                <div className={`p-3 rounded-lg bg-muted mr-4 ${action.color}`}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage your shipments and addresses from this dashboard
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-sm text-muted-foreground space-y-2">
            <p>Use the sidebar navigation or quick actions above to:</p>
            <ul className="list-disc list-inside space-y-1 ml-2">
              <li>Create and track shipments</li>
              <li>Manage your pickup/return addresses</li>
            </ul>
          </div>
        </CardContent>
      </Card>
...
export default function DashboardPage() {
  ...
  return (
    <ProtectedRoute>
      <DashboardLayout>
        <PageContainer>
          {isOutlet ? <OutletDashboard user={user} /> : <AdminDashboard />}
        </PageContainer>
      </DashboardLayout>
    </ProtectedRoute>
  );
