...
Replaced the placeholder `/dashboard` (which only showed partner/zone/charge-definition counts) with a real operations/financial dashboard covering shipments, outlets, and wallet/financial data — no mock data, everything backed by new aggregation endpoints.

**Backend — new read-only aggregation endpoints, no schema changes:**

- shipment-service, `backend/shipment-service/controllers/dashboardController.js` (new), routes wired in `routes/shipments.js` ahead of `/:id`:
  - `GET /api/v1/shipments/dashboard/summary?days=30` — totals, status/shipmentType breakdown, NDR/RTO/delivered rates, `exceptions.hold`, avg TAT (`actualDelivery - createdAt` for DELIVERED rows), booking failures (`bookingStatus === "PENDING_BOOKING"`), revenue/courierCost/profitMargin (flagged provisional via `courierCostPendingCount`), COD collectable by status bucket (settlement status explicitly not knowable here — lives in wallet-service).
  - `GET /api/v1/shipments/dashboard/trend?days=30` — daily {date,count,revenue}, bucketed in JS (no raw SQL date_trunc).
...
- shipment-service's 5 endpoints cache ~90s via `shared/lib/redis.js`; wallet-service's 2 endpoints cache ~90s the same way but around external-API calls (arguably even more valuable there), keyed by route + caller identity + query params, degrading to live queries on Redis failure.
- No API Gateway changes needed — `/api/v1/shipments/*` and `/api/v1/wallet/*` proxy prefixes already cover the new `/dashboard/*` sub-routes.

**Frontend** — `frontend/src/app/dashboard/page.tsx` rebuilt (replaces the old `page.jsx`, which was deleted — watch for stray duplicate-page warnings if it's ever reintroduced by mistake) using `PageHeader/PageContainer/StatsGrid/StatsCard/DataTable` conventions from `charge-configs/page.tsx`. New `frontend/src/components/dashboard/` (`dashboard-charts.tsx`, `chart-empty-state.tsx`, `formatters.ts`). Added **recharts@^3.10.1** as the project's first chart library (`yarn add recharts` — install inside the frontend container, not the host, since the host has no `node_modules`). Chart colors are theme-aware via new `--chart-1..8` / `--chart-good/warning/serious/critical` / `--chart-grid` / `--chart-axis` CSS variables added to `globals.css` (light+dark blocks) — components reference `var(--chart-*)` directly, never hardcoded hex. New RTK Query endpoints added to `shipmentApi.ts` and `walletApi.ts` (exactly 7 hooks total on the page, no per-row fetches). Role-adaptive: admin/client roles get the full platform view (top-outlets leaderboard, low-balance wallets table); outlet role gets the same KPI/chart shapes reframed to "your" data with those two platform-wide tables hidden (the backend already scopes them to a single row for outlets, so hiding is a UX choice, not a security boundary).

**Verification:** code review passed 11/11 rule-compliance checks (controller pattern, Prisma-only/no raw SQL, Joi validation actually enforced, tenant scoping on every endpoint, no audit logging needed since read-only, no mock data, theme-aware colors, RTK Query conventions, no N+1). Curl-tested all 7 endpoints through the gateway with both a superadmin token and an outlet token (`sales@subsolution.in`) confirming scope isolation (outlet never sees platform-wide numbers). `npx tsc --noEmit` clean inside the frontend container (only the pre-existing known `GetNDRCasesParams`/`GetBulkJobsParams` errors in `shipmentApi.ts` remain, unrelated). Docker logs clean across shipment-service/wallet-service/frontend/api-gateway after a joint restart; a real browser load of `/dashboard` was confirmed end-to-end in the gateway logs (all 7 calls 200).

One bug was found by review and fixed: `WalletDashboardLowBalanceRow` in `walletApi.ts` was hand-typed with wrong field casing (`user_id`/`client_code`/`currency`) that didn't match the backend's actual camelCase payload (`userId`/`clientCode`, no `currency`) — silently blanked the Client column in the low-balance table. Fixed to match the real shape.

**Known gaps (not silently faked):** shipment-service cannot determine true COD _settlement_ status (money actually remitted) — that state lives in wallet-service (as external-API data) and wasn't wired cross-service for this pass. The shipment-side dataset in this environment is tiny (3 shipments, 1 courier with data) so empty states matter — they're deliberately designed (see `chart-empty-state.tsx`), not accidental blanks. Wallet data, once correctly sourced from the external API, is real and non-trivial (2 wallets, ₹2664.80 total, 19 transactions for the `LOGISTICS` client).

**Unrelated pre-existing bug found, NOT fixed (separate task):** `GET /api/v1/shipments?page=abc` returns 500 instead of 400 — `shipmentController.js` around line 1937 passes a non-numeric `page` straight into Prisma's `skip` with no guard.

**Housekeeping note:** outlet account `sales@subsolution.in`'s password was reset during scope-isolation testing (pre-authorized for this task specifically) — its temporary password is `Outletd73435bfc41069bb@123!`; communicate this to the account owner or have them reset it again.

### 🧾 Shipment Tax Invoice System (P0 - Completed August 22, 2026)

Built a proper GST tax invoice system on top of shipment-service, distinct from the pre-existing `ShipmentInvoice` model (the customer's commercial invoice/e-way-bill captured at booking — untouched). New `Invoice` and `InvoiceCounter` Prisma models back **financial-year-aware, gap-free invoice numbering** — `INV/{FY}/{seq}` for tax invoices, `CN/{FY}/{seq}`/`DN/{FY}/{seq}` for credit/debit notes, each series with its own counter row — implemented in the new `services/invoiceService.js`.

**Concurrency safety.** `issueTaxInvoice` is idempotent and safe under concurrent issuance for the same shipment: a partial unique index `invoices_one_tax_invoice_per_shipment` on `(shipment_id) WHERE invoice_type='TAX_INVOICE' AND status='ISSUED'` is the backstop, caught via Prisma's `P2002` and resolved by returning the winning row rather than erroring. Prisma 5.22's `schema.prisma` can't express a partial index, so it was added by hand-writing the migration SQL (`20260822120000_add_one_tax_invoice_per_shipment_index`, following `20260822095413_add_tax_invoice_system`).

**Adjustment notes.** `issueAdjustmentNote` issues credit/debit notes for re-rates and is hooked into `rerateShipment` in `shipmentController.js` as a **best-effort try/catch** — a note-issuance failure never rolls back the wallet adjustment that already happened.

**GST data flow.** New `services/gstStateCodeMap.js` hardcodes the India state/UT → GST state code table. shipment-service snapshots GSTIN/company-name/address onto the invoice at issue time via two new internal endpoints on user-service — `GET /api/v1/internal/outlets/:outletId/billing-details` and `GET /api/v1/internal/clients/:clientId/billing-details` (protected by `requireInternalRequest`, same as existing internal routes) — and never re-fetches them afterward, so an invoice reflects the billing details as they stood at issuance even if the outlet/client edits them later. **Known gap**: the `Client` Prisma model in user-service has no GSTIN field at all, so client-billed invoices always have `billedGstin: null` — a real data-model gap (not a bug), flagged for a future task if client-side GST invoicing becomes a real requirement.

**PDF generation.** `services/invoicePdfService.js` renders real PDFs via `pdfkit` (new dependency), stored under `generated/invoices/` following the same convention as `labelGenerationService.js`.

**API surface** — `controllers/invoiceController.js`, `validation/invoiceSchemas.js`, `routes/invoices.js`, mounted at `/api/v1/invoices` in `server.js`: `POST /api/v1/invoices/shipments/:shipmentId/issue`, `GET /api/v1/invoices/:id`, `GET /api/v1/invoices` (filtered/paginated list), `POST /api/v1/invoices/notes` (body `{financialAdjustmentId}`), `GET /api/v1/invoices/:id/pdf`. api-gateway's service proxy map gained an `invoices` entry routing `/api/v1/invoices` → shipment-service:3004.

**Frontend** — `store/api/endpoints/invoiceApi.ts` (RTK Query: `issueInvoice`/`getInvoice`/`listInvoices`/`issueNote`/`downloadInvoicePdf`), new `app/wallet/invoices/page.tsx` (list) and `app/wallet/invoices/[id]/page.tsx` (full detail: GST breakdown, related credit/debit notes, PDF download) — the latter **replaces** the old mock-data stub `page.jsx`, which was deleted.

**Honest limitations, recorded rather than hidden:**

- **Place of supply**: the invoice treats `billingState` as place of supply and `pickupState` as the supplier's state — a reasonable B2B freight-services GST reading, but the platform has no explicit "registered supplier state per invoice" concept anywhere else. Flagged in code comments in `invoiceService.js`; should be confirmed by an accountant before this is used for real GST filings.
- **Platform GSTIN/company details** (`PLATFORM_GSTIN`, `PLATFORM_COMPANY_NAME`) are placeholder env vars with no real values configured yet — the PDF renders clearly-marked placeholders until they're set.
- **List endpoint has no `shipmentId` filter** — the detail page's "related notes for this shipment" feature works around this by filtering client-side after fetching by outlet/client, which won't scale; a real `shipmentId` query param should be added to `GET /api/v1/invoices` in a follow-up.
- **Rounding**: CGST/SGST 50/50 splits assign any odd-paisa rounding remainder to the SGST leg (`sgstAmount = gstTotal - cgstAmount`) so the two always sum exactly to the GST total.
- ⚠️ **Dev-environment wallet data quality issue discovered during this work**: no shipment in the dev DB had a real `walletTransactionId` set at time of building (wallet integration appears broken in this environment, consistent with the recent `fix: wallet_issue_during_shipment_creation` commit) — verification required patching test fixtures directly via Prisma. Pre-existing environment issue, not introduced by or in scope for this task.

### 🔌 External Shipment API + Developer Docs + MCP Server (P0 - Completed August 21, 2026)

Shipped a public, credential-authenticated **External Shipment API** so outlets can integrate booking/tracking into their own systems, plus a `/developers` documentation-and-credential-manager in the portal and a standalone stdio **MCP server**. The API surface _reuses the existing shipment controllers verbatim_ — the new layer is auth, scope, rate limiting, idempotency, a stable public envelope, and a one-step booking convenience.

**Credential model (auth-service).** New `ApiCredential` model: `clientId` (`lgk_live_…`) + bcrypt `secretHash` (+`secretLast4`), `scopes[]`, `ipAllowlist[]`, `rateLimitPerMin`, `pinnedVersion`, `expiresAt`/`revokedAt`. **A credential freezes exactly one acting principal at creation** (`actingUserId`, `actingPhone`, `tenantId`, `outletId`) — resolved once via user-service internal lookup, never per request. If `outletId` is set, tokens always mint as `role: "outlet"` for that outlet regardless of who created the key, which is _why the existing controllers needed no changes_: `applyScopeFilter()` scopes the data and `resolveOutletContext()` finds its wallet context with zero body params. Only a platform-level (no `outletId`) credential keeps `role: superadmin` and must name `outletId`/`outletUserId` per booking. `POST /api/v1/external/auth/token` mints a 1h JWT with `aud: "external-api"`, `jti`, `apiCredentialId`, `scopes`; **no refresh token** and **no `session:` Redis key** (see below). Credential CRUD lives at `/api/v1/api-credentials` (auth-service), deliberately _outside_ `/api/v1/external`.

**Four load-bearing security findings, all fixed:**

1. **Token confinement (critical).** Every JWT is signed with the same `JWT_SECRET`, and `shared/lib/auth.js:authenticate` accepts any of them — the only thing that had been stopping an API token from hitting `/api/v1/wallet/*` or `/api/v1/users/*` was the Redis `session:<uid>` check, which _passes_ whenever the same outlet is also logged into the dashboard. An API key leak would have been full account takeover. The gateway (`authValidator.js`) now enforces **audience↔path binding** in both directions: `aud=external-api` outside `/api/v1/external/*` → 401 `EXTERNAL_TOKEN_PATH_SCOPE`; a session token _inside_ it → 401 `SESSION_TOKEN_NOT_ALLOWED`. `isPublicPath` was also tightened from a bare `startsWith` to `path === p || startsWith(p + "/")` so a public prefix can't whitelist siblings.
2. **Downstream hops.** shipment-service forwards the _caller's_ `Authorization` to partner-service and wallet-service, which re-run `authenticate` — so an external token (with no `session:` key) 401'd on both quoting and wallet debit. `authenticate` now branches on `aud === "external-api"` and checks `apicred:revoked:<id>` / `apitoken:revoked:<jti>` instead of the session key. Permission caches (`perm:`, `perms:`) are namespaced by `apiCredentialId` so a scoped credential can never inherit its user's full-permission dashboard cache.
3. **AWB enumeration.** `trackByAwbNumber` (`shipmentController.js:4728`) has **no** `applyScopeFilter` — it returns tracking for any AWB in the system. The external adapter proves ownership via `applyScopeFilter` before delegating.
4. **Double debit.** `createShipment` debits the wallet (`:562`) _before_ the insert (`:648`) and its catch block does not refund; `@@unique([orderId, clientId])` fires after the money moves and — confirmed against live data — **enforces nothing for outlets, whose `clientId` is NULL**. Added `@@unique([orderId, userId])` and an `IdempotencyRecord` table reserved _before_ any side effect.

**Two pre-existing blockers found and fixed** (both would have broken outlet booking in the portal too; they were invisible because all prior verification used a superadmin token, which short-circuits `checkPermission`):

- Outlets had **no `partner:read:own`**, but partner-service's `/calculate` requires it → _no outlet could ever fetch a quote_. Added to `DEFAULT_ROLE_PERMISSIONS.outlet`, to the permission seed, and upserted into the live DB. (Note `getEffectivePermissions` always falls back to JWT `permissions[]` because it sends the literal string `"X-Internal-Request": "true"`, which every service's gate rejects — so the constants file is the effective source of truth.)
- Wallet money-movement (`/admin/wallet`, `/admin/debit`, `/admin/refund`) was authorized against the **caller's** `wallet:manage:all`, which no outlet holds → booking impossible for every non-admin role. Added `authMiddleware.requirePermissionOrService()`: a service proves itself with `X-Service-Token === INTERNAL_SECRET`, otherwise the normal permission check runs. The gateway now **strips** `x-service-token`, `x-service-name`, `x-internal-request` and all `x-user-*` headers from inbound client requests, so they can only originate on the internal network (spoofing verified blocked). The reporting/admin wallet endpoints keep the strict permission gate.

**API surface** — `/api/v1/external/shipments`, middleware order `envelope → externalAuth → rate limit → scope → idempotency → validate` (the limiter must follow auth, unlike the internal routes where it runs first and keys on `ip:anonymous`): book, list, details, edit, cancel, rates, serviceability, tracking, track-by-AWB, documents, label. Details/edit/cancel/tracking accept **shipment id, AWB, or the caller's own orderId**. New `services/quoteService.js` lifts the quote logic out of `getShipmentQuotes` verbatim (`shipmentController.js:3768-3908`) so booking and quoting sign tokens in one place. **One-step booking**: omit `quoteToken` and pass `selection: cheapest|fastest` (or a `partnerId`) — the server quotes, picks, signs and books in a single call. Its quote params are derived once from the exact post-coercion body fields `assertClaimsMatchPayload` later compares against; recomputing them differently is how a one-step booking fails its own signature. Because the token is seconds old, one-step booking can never return `QUOTE_STALE`. Public envelope is `{success, data, meta, request_id}` with a stable snake_case error taxonomy (`insufficient_scope`, `quote_stale`, `idempotency_key_reuse`, …) — the internal `APIResponse` shape is never leaked, including from the gateway's own auth failures.

**Frontend** — `/developers` (docs: overview, quickstart, per-endpoint reference with curl/Node/Python samples, errors, rate limits, MCP setup) and `/developers/credentials` (create → secret shown once, rotate, revoke). Endpoint reference is authored as **data** in `src/docs/external-api/endpoints.ts`, so the side nav and sections render from one array. New `components/developers/code-block.tsx` has copy + language tabs and a ~20-line regex highlighter — **deliberately zero new dependencies** (`next.config.js` disables TS/ESLint during builds, so a new dep is the easiest way to break the build unnoticed). Uses the project's `useToast` hook, **not** `react-toastify` (imported by one older page but not actually installed — it fails `type-check`). Sidebar entry under My Account (`superadmin`, `outlet`); `middleware.ts` gained `/developers` in both `AUTH_REQUIRED_ROUTES` and `ROLE_RESTRICTED_ROUTES`.

**MCP server** — new `mcp-server/` workspace package (`@modelcontextprotocol/sdk`, stdio, ESM), 9 tools: `get_quotes`, `book_shipment` (one-step, auto idempotency key), `list_shipments`, `get_shipment`, `track_shipment`, `edit_shipment`, `cancel_shipment`, `recalculate_rates`, `check_serviceability`. Caches the token for its hour, refreshes once on 401, surfaces the API's `code` + `request_id` on failure. Verified live against the running stack via a real MCP client: all 9 tools listed, quoting returned ₹462.97, and cross-tenant access correctly errored through the MCP layer.

**Migrations** (via the repo's `migrate diff → db execute → migrate resolve` procedure — `prisma migrate dev` is still unusable): `20260821090827_add_api_credentials` (auth-service), `20260821090842_add_idempotency_records` (shipment-service, + `shipments_order_id_user_id_key`).

**Verification** — 12/12 curl checks green (token exchange, bad-secret 401, all four containment directions, list/rates/serviceability, validation 400, unknown-shipment 404). Tenant isolation verified on **five** vectors (id, orderId, AWB, tracking, documents) — all 404. Idempotency verified: replay with `Idempotency-Replayed: true`, 422 on key reuse with a different body, and reservation released on failure so real retries work. Scope enforcement verified with a read-only credential (403 `insufficient_scope` on book/cancel). Revocation kills live tokens immediately. Rate limiter trips at the per-credential ceiling with `Retry-After`. Tests: 25 shipment-service (11 new for `deriveQuoteParams`/`selectQuote`), 30 partner-service, all passing. Frontend `type-check` clean on every new file (only the pre-existing 14 `shipmentApi.ts` `Params | void` errors remain); production build compiled with `/developers` and `/developers/credentials` present.

⚠️ **Booking could not be completed end-to-end in this environment**: the chain is verified all the way through quoting, partner selection, token signing and the wallet-service authorization hop, but the **third-party wallet API (`wapi.websiteduniya.com`) rejects this machine's public IP as not whitelisted** (`Access denied: IP address not in whitelist`). Same blocker recorded in the Aug 4 session. Re-run the one-step booking test from a whitelisted host to close the loop.
